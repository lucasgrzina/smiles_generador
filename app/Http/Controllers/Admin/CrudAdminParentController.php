...
class CrudAdminParentController extends AppBaseController
{
    ...
    public function index($parentId)
    {
        $this->data = [
	            ...
	            'loading' => true,
	            'url_filter' => route($this->routePrefix.'.filter',[$parentId]),
	            'url_create' => route($this->routePrefix.'.create',[$parentId]),
	            'url_edit' => route($this->routePrefix.'.edit',[$parentId,'_ID_']),
	            'url_show' => route($this->routePrefix.'.show',[$parentId,'_ID_']),
    ...
    public function create($parentId)
    {
        $this->data = [
            ...
            'perms' => auth()->user()->getAllPermissions()->pluck('name'),
            'roles' => auth()->user()->getRoleNames(),
            'url_save' => route($this->routePrefix.'.store',[$parentId]),
    ...
    public function show($parentId,$id)
    {
       $model = $this->repository->findWithoutFail($id);
        ...
        if (empty($model)) {
            return redirect(route($this->routePrefix.'.index',[$parentId]));
        }
